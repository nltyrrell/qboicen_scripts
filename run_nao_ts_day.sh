...
# This script runs the CDO commands for all models, using nnao_master.sh
# then combines all timeseries into one CSV file per model

#
#============
# Use nao_master.sh to get the timeseries for all models
# nao_master.sh.sh script is:
...
$script_dir/nao_master.sh --group=$group --model=$model --exp=$exp --tmean=$tmean --real=$real --punits=$punits --years=$years --model_dir=$model_dir --model_filename=$model_filename --lat_size=$lat_size --actor_dir=$actor_dir --make_actor_dir=$make_actor_dir
...
if [ "$tmean" == "day" ]; then
	merge_time=true
	model_filename="${tmean}_CMAM_QBOi${exp}_${real}_*"
fi

$script_dir/nao_master.sh --group=$group --model=$model --exp=$exp --tmean=$tmean --real=$real --punits=$punits --years=$years --model_dir=$model_dir --model_filename=$model_filename --lat_size=$lat_size --actor_dir=$actor_dir --make_actor_dir=$make_actor_dir --merge_time=$merge_time
...
if [ "$tmean" == "day" ]; then
	...
	model_filename="${tmean}_UMGA7_QBOi${exp}_${real}_*"
	merge_time=true
fi
actor_dir=""
make_actor_dir=true
